...
""" And event handler return value which can trigger an action or switch active handlers.

If a handler is returned then it will become the active handler for future events.
If an action is returned it will be attempted and fi it's valid then
...
class AreaRangedAttackHandler(SelectIndexHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        # Draw a rect around the targeted area, so the player can see the affected area
        console.draw_frame(
            x=x - self.radius - 1,
            y=y - self.radius - 1,
            width=self.radius ** 2,
